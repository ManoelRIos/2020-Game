...
import com.apstudios.entities.Bullet;
import com.apstudios.entities.Enemy;
import com.apstudios.entities.Entity;
import com.apstudios.entities.Lifepack;
...
public class World {
		...
		try {
			...
			for(int xx = 0; xx < map.getWidth(); xx++){
				for(int yy = 0; yy < map.getHeight(); yy++){
					...
					if(pixelAtual == 0xFF000000){
					...
					}else if(pixelAtual == 0xFF1f00ff) {
						//Player
						Game.player.setX(xx*16);
						Game.player.setY(yy*16);
					}else if(pixelAtual == 0xFF00ff27) {
						//Player2
						...
						buf[3] = Game.spritesheet.getSprite(48,32,16,16);
						Enemy en = new Enemy(xx*16,yy*16,16,16,buf);
						Game.entities.add(en);
						Game.enemies.add(en);
						
					}else if(pixelAtual == 0XFFff00f7) {
						//Life Pack
						Lifepack pack = new Lifepack(xx*16,yy*16,16,16,Entity.LIFEPACK_EN);
						Game.entities.add(pack);
					}else if(pixelAtual == 0xFFf0ff00){
						//Bullet
	...
	public static void restartGame(String level){
        ...
        Game.player2 = new Player2(0,0,16,16,Game.spritesheet.getSprite(32, 0,16,16));
